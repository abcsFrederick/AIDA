...
const AIDA = () => {
	...
	let imageID = ''
	if (query.imageId !== undefined) {
		imageID = Array.isArray(query.imageId) ? query.imageId[0] : query.imageId
	}
	...
	useEffect(() => {
		;(async () => {
			if (router.isReady) {
				if (imageID !== '') {
					const dataHost = `https://${config.server.hostname}/${config.server.path}/data/`
					setImageUrl(`${dataHost}${imageID}`)
					setImageExt(imageID.split('.')[1])

					try {
						const annotationPath = imageID.replace(/\.[^.]+$/, '.json')
